...
def plot_grid(pois_df,height,use_area):
    ...
    grid_df.plot(ax=city_plot,column='value',cmap=my_cmap,vmin=0 ,k=5,edgecolor='white',linewidth=0.1,legend=True, categorical=categorical)
    return grid_df
...
# def make_folium_hotspot_map(grid_df,city_boundary_df):
#     m = folium.Map([40.4406, -79.9959], zoom_start=12)

    
#     def style_function(x): 
#     #     print(x)
#         # color = matplotlib.colors.to_hex(cm(x['properties']['value']),keep_alpha=True)
#         return     {
#     #             'fillColor': color,
#                 'fillColor': 'Red' if (x['properties'][col])=='hot spot',
#     #             'color': 'white',
#                 'weight': 1,
#                 'dashArray': '10, 10',
#                 'fillOpacity': .9,
...
def analyze(pois,filename,height,run_query=True,use_area=False):
    pois_df,pois_plot,city_boundary_df = plot_cluster(pois,f'{filename}.osm',f'{filename}.geojson',run_query=run_query)
...
def analyze_folium(pois,filename,height,threshold,run_query=True,use_area=False):
    pois_df,m0,city_boundary_df = plot_cluster_folium(pois,f'{filename}.osm',f'{filename}.geojson',run_query=run_query)
